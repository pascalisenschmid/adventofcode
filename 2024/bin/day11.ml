...
let transform list =
  List.fold list ~init:[] ~f:(fun acc n ->
...
let transforms amount map =
  Advent.range_seq 1 amount |> Seq.fold_left (fun acc x -> transform2 acc) map
;;
...
let () =
  ...
  Map.fold (fun k v acc -> acc + v) part1 0 |> Fmt.pr "\nPart 1: %d";
  Map.fold (fun k v acc -> acc + v) part2 0 |> Fmt.pr "\nPart 2: %d"
;;
